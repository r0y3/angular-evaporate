    ...
    .provider('eva', function () {
      ...
      this.$get = [function () {
        return {
          ...
          urlBase: 'http://' + options.region + '.s3.amazonaws.com/' + options.bucket + '/'
        };
      }];
    })

    .directive('evaporate', ['eva', 'Slug', function (eva, Slug) {
        ...
        if (eva._.supported) {
          ...
          element.bind('change', function (event) {
            ...
            angular.forEach(event.target.files, function (file) {
              ...
              file.path_ = dir + (new Date().getTime()) + timestampSeparator + Slug.slugify(name) + '.' + ext;
